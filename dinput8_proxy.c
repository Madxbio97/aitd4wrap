 ...
 * v10 - Custom audio engine replacing MSS/mss32.dll
 *
 * Audio engine features:
 *   - IMA ADPCM decoder (correct implementation)
 *   - Software mixer: 16 samples + 4 streams
 *   - Linear-interpolation resampler (any rate -> 44100 Hz)
 *   - waveOut quad-buffered output at 44100/16/stereo
...
typedef struct {
    ...
    /* Decode buffer (decoded PCM, 16-bit) */
    short *decode_buf;
    ...
    for (int si = 0; si < AE_MAX_SAMPLES; si++) {
        ...
        for (int f = 0; f < outFrames; f++) {
            ...
            if (ipos >= s->pcm_frames) {
                ...
                /* Start crossfade to prevent click */
                s->loop_fade = 16;
            }

            /* Point sampling (matches MSS 4.x default - no interpolation filter) */
            int smpL, smpR;
            if (s->pcm_channels == 1) {
                int v = s->pcm_data[ipos];
                smpL = (v * volL) / 127;
                smpR = (v * volR) / 127;
            } else {
                int vL = s->pcm_data[ipos * 2];
                int vR = s->pcm_data[ipos * 2 + 1];
                smpL = (vL * volL) / 127;
                smpR = (vR * volR) / 127;
            }
            ...
            if (s->loop_fade > 0) {
                int t = 16 - s->loop_fade; /* 0..15 */
                smpL = (s->prev_smpL * (16 - t) + smpL * t) / 16;
                smpR = (s->prev_smpR * (16 - t) + smpR * t) / 16;
                s->loop_fade--;
            }
            s->prev_smpL = smpL;
            s->prev_smpR = smpR;
        ...
        for (int f = 0; f < outFrames; f++) {
            ...
            if (ipos >= st->decode_buf_frames) {
                ...
                if (st->data_read >= st->data_size) {
                    ...
                    /* Loop: reset read position */
                    st->data_read = 0;
                }
                /* Decode next chunk from memory */
            ...
            int smpL, smpR;
            if (st->channels == 1) {
                int v = st->decode_buf[ipos];
                smpL = (v * volL) / 127;
                smpR = (v * volR) / 127;
            } else {
                int vL = st->decode_buf[ipos * 2];
                int vR = st->decode_buf[ipos * 2 + 1];
                smpL = (vL * volL) / 127;
                smpR = (vR * volR) / 127;
            }
 ...
 * Called from the mixer when data is needed.
 */
static void AE_StreamFillBuffer(AE_Stream *st) {
    int frames;
...
static MSS_S32 __stdcall Hook_AIL_startup(void) {
    if (g_config.use_custom_audio) {
        /* Initialize real MSS too - needed for timer subsystem (register/start/stop_timer) */
        if (real_AIL_startup) {
            MSS_S32 r = real_AIL_startup();
            LogMsg("AE: real AIL_startup -> %d (for timer subsystem)\n", r);
        }
        if (!g_aeInitialized) {
            ...
            g_aeInitialized = 1;
            LogMsg("AE: AIL_startup -> custom engine initialized\n");
        }
        return 1;
    }
    /* Pass-through */
...
/* Timer functions - pass through to real MSS (they don't use sample handles) */
...
static MSS_U32 __stdcall Hook_AIL_register_timer(void *callback) {
    if (g_config.use_custom_audio) {
        /* Timer services: pass to real MSS - timers don't use sample handles */
        if (real_AIL_register_timer) return real_AIL_register_timer(callback);
        return 0;
    }
...
static void LoadConfig(void) {
    ...
    if (g_config.enable_log) {
        ...
        LogMsg("=== AITD Wrapper v10 (Custom Audio Engine) ===\n");
...
BOOL WINAPI DllMain(HINSTANCE hDll,DWORD reason,LPVOID reserved){
    ...
    if(reason==DLL_PROCESS_ATTACH){
        ...
        if(g_config.show_diagnostics){
            char m[512];
            snprintf(m,sizeof(m),"AITD Wrapper v10\n\nAR=%d CD=%d TF=%d TC=%d\n"
                "Windowed=%d MSAA=%d\nRes=%dx%d %s\nCustom Audio=%d",
                g_config.fix_aspect_ratio,g_config.fix_color_depth,
                g_config.fix_texture_filter,g_config.fix_texture_clamp,
                g_config.windowed,g_config.msaa_samples,
